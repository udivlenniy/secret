...
class NestedSetBehavior extends CActiveRecordBehavior
{
     ...
     */
    public function countChildrenInIntervalLevels($levelStart, $levelEnd){
        ...
        $level = ' `t`.`level`=>'.($owner->{$this->levelAttribute}+$levelEnd).'  AND `t`.`level`<='.($owner->{$this->levelAttribute}+$levelStart).'';

        $sql = 'SELECT COUNT(id) AS count FROM `tbl_partner` `t`
                WHERE ((`t`.`lft`>'.$owner->{$this->leftAttribute}.'
                    AND `t`.`rgt`<'.$owner->{$this->rightAttribute}.')
                      AND `t`.`level`<='.($owner->{$this->levelAttribute}+$levelEnd).'  AND `t`.`level`<='.($owner->{$this->levelAttribute}+$levelStart).'
                    )
                ORDER BY `t`.`lft`';

        $result = $connect->createCommand($sql)->queryRow();

        return $result['count'];
    }


    public function countChildren($status='', $deep=''){
        ...
        if(!empty($deep)){
            $level = ' AND (`t`.`level`<='.($owner->{$this->levelAttribute}+$deep).')';
        }else{
            //$level = ' AND (`t`.`level`<='.($owner->{$this->levelAttribute}).')';
            $level = '';
        }
