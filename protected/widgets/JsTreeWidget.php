...
EOD;
    ...
    {
        $partners = CActiveRecord::model($this->modelClassName)->findAll(array('order' => 'lft'));
        $partners = CActiveRecord::model($this->modelClassName)->findAll(array('order' => 'lft','condition'=>'root=95'));
        //$partners = $partners_->children()->findAll();
        $identifiers = array();
        foreach ($partners as $n => $partner) {
            $identifiers[] = "'" . 'node_' . $partner->id . "'";
        }
